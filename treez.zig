...
pub const CursorWithValidation = struct {
    ...
    pub const TextPredicate = union(enum) {
        equal_predicate: EqualPredicate,
        match_predicate: MatchPredicate,
    };

    pub const PredicateList = std.ArrayListUnmanaged(TextPredicate);
    ...
    pub fn init(allocator: std.mem.Allocator, query: *const Query) !CursorWithValidation {
        ...
        for (0..query.getPatternCount()) |pattern| {
            ...
            while (index < preds.len) {
                const pred = query.getStringValueForId(@as(u32, @intCast(preds[index].value_id)));

                switch (std.meta.stringToEnum(Predicates, pred).?) {
                    .@"eq?" => {
                        ...
                        //     // std.debug.print("predicate: {s}", .{query.getStringValueForId(@as(u32, @intCast(preds[index].value_id)))});
                    ...
                    // else => @panic("Only the 'eq?' and 'match?' predicates are supported by treez at the moment."),
                }
    ...
    pub fn isValid(validator: CursorWithValidation, source: []const u8, match: Query.Match) bool {
        if (validator.predicate_map.get(match.pattern_index)) |pred_loc| {
            const predicates: []const TextPredicate = validator.predicates.items[pred_loc.index .. pred_loc.index + pred_loc.len];
            for (predicates) |pred| {
                switch (pred) {
                    .equal_predicate => |val| {
                        const a = validator.capture_name_to_id.get(val.a).?;
                        const b_capture = switch (val.b) {
                            .string => null,
                            .capture => |c| validator.capture_name_to_id.get(c).?,
                        };

                        var a_value: ?[]const u8 = null;
                        var b_value: ?[]const u8 = switch (val.b) {
                            .string => |v| v,
                            .capture => null,
                        };
